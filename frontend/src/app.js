document.addEventListener('DOMContentLoaded', () => {
  ...
  socket.on('new', (data) => {
      ...
      document.getElementById('greeting').innerHTML = `Group ${data.group} - ${data.name}.<span id="disappear"> <br><br/>  ID is ${data.group}. Give the code to enemy </span>`;
      currentTurn = null;
  });
  ...
  socket.on('player2', (data) => {
      ...
      currentTurn = false;
      document.getElementById('turn').textContent = 'Waiting for your enemy...';
  });
  ...
  const handleBoxClick = (e) => {
      ...
      if (currentTurn == null) {
          alert('Your enemy has not entered.');
          return;
      }
      if (!currentTurn) {
